    ...
    <footer id="contato" className="relative z-10 mt-auto border-t border-white/5 bg-brand-dark/80 text-center backdrop-blur-md">
      <div className="mx-auto max-w-4xl px-6 py-16">
        ...
        <div className="flex flex-col items-center justify-center gap-4 font-mono text-sm text-gray-300 md:flex-row md:gap-6">
          <a href={`mailto:${profile.email}`} className="rounded border border-white/5 bg-white/5 px-4 py-2 transition-colors hover:border-brand-red/50 hover:text-white">✉ {profile.email}</a>
          <a href={`tel:${profile.phone.replace(/\D/g, "")}`} className="rounded border border-white/5 bg-white/5 px-4 py-2 transition-colors hover:border-brand-red/50 hover:text-white">📱 {profile.phone}</a>
        </div>
